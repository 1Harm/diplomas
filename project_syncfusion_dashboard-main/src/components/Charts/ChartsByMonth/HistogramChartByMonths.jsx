import { ChartComponent, SeriesCollectionDirective, SeriesDirective, Inject, HistogramSeries, Category, Legend, Tooltip } from '@syncfusion/ej2-react-charts';
...
const HistogramChart = () => {
  ...
  return (
    <div>
      ...
      >
        <Inject services={[HistogramSeries, Category, Legend, Tooltip]} />
        <SeriesCollectionDirective>
          {filteredRevenueData.map((revenue, index) => (
            <SeriesDirective key={index} dataSource={revenue} xName="month" yName="total" name={`Компания ${index + 1}`} type="Histogram" width={2} marker={{ visible: true }} />
          ))}
        </SeriesCollectionDirective>
      </ChartComponent>
    </div>
